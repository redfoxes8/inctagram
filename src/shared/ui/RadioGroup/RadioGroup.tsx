...
function RadioGroupComponentInner(
  ...
  return (
    ...
    >
      {options.map((option) => {
        const itemId = `${generatedId}-radio-${option.value}`
        return (
          <div key={itemId} className={classNames.optionWrapper}>
            <RadioGroupRadix.Item className={classNames.item} id={itemId} value={option.value}>
            ...
            <label className={classNames.label} htmlFor={itemId}>
              {option.label}
            </label>
          </div>
        )
